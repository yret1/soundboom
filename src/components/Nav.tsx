...
const Nav : React.FC<NavOptionsProps> = ({setCurrent}) => {
  ...
  return (
    <header className='bg-secondary-100 h-20 flex justify-center pl-5 relative items-center lg:px-20'>
        <div onClick={() => {setOpen(!open)}} className='h-full w-12 flex justify-start items-center lg:hidden'>
        ...
        <nav className='flex-1 hidden lg:block'>
            <ul className='flex justify-center gap-10 items-center font-manrope tracking-2 font-bold text-white'>
                ...
                <Link to={"/category"} onClick={() => setCurrent("Headphones")} className='hover:text-primary-100 cursor-pointer'>HEADPHONES</Link>
                <Link to={"/category"} onClick={() => setCurrent("Speakers")} className='hover:text-primary-100 cursor-pointer'>SPEAKERS</Link>
                <Link to={"/category"} onClick={() => setCurrent("Earphones")} className='hover:text-primary-100 cursor-pointer'>EARPHONES</Link>
            </ul>
        </nav>
